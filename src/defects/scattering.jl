...
function scattering_atoms(imps::Vector{ImpurityState}, mod_atoms::Vector{PerturbedAtom})
    ...
    all_neighbors = collect(Iterators.flatten(map(neighbors, map(x -> x.coord, mod_atoms))))
    # all_neighbors = reduce(vcat, map(neighbors, map(x -> x.coord, mod_atoms)))
    ...
    hosts_neighbors =
        Vector{PerturbedAtom}(filter(x -> x.coord ∉ mod_coords, hosts_neighbors))
