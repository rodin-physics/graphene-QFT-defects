...
function Hπ(q)
    ...
    return ([
        (t0+t2*f2+t5*f5+t6*f6+t10*f10+t12*f12+t15*f15+t17*f17) (t1*f1+t3*f3+t4*f4+t7*f7+t8*f8+t9*f9+t11*f11+t13*f13+t14*f14+t16*f16)
        conj(t1 * f1 + t3 * f3 + t4 * f4 + t7 * f7 + t8 * f8 + t9 * f9 + t11 * f11 + t13 * f13 + t14 * f14 + t16 * f16) (t0+t2*f2+t5*f5+t6*f6+t10*f10+t12*f12+t15*f15+t17*f17)
    ])
end

# Green's function matrix for the 17-parameter TB model
function Gπ(z, q)
    return inv([1 0; 0 1] * z - Hπ(q))
end
